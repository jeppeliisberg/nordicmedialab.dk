...
interface CarouselProps {
  cards: { id: number; headline: string, text:string; logo?: string; hero: string, projecturl?:string}[];
}

const ProjectsSwiper: FC<CarouselProps> = ({ cards }) => { 
        ...
        {cards.map((card) => (
            <SwiperSlide key={card.id} className='cursor-grab'>
              <div className="relative select-none flex flex-col h-full w-full mx-auto rounded-2xl overflow-hidden bg-gray-900">
                  <div className='w-auto aspect-[4/3]'>
                    <img
                        src={card.hero}
                  ...
                  <div className="bg-gray-900 flex flex-col gap-y-3 items-start justify-between w-full px-4 py-3 h-full text-white">
                    <div className="block">
                      <h1 className={`font-serif text-orange-400 font-semibold text-xl lg:text-2xl mt-2 mb-3`}>
                          {card.headline}
                      </h1>
                      
                      <p className='text-sm font-sans'>{card.text}</p>
                      {card.projecturl != '' &&
                      <a href={card.projecturl} className='text-sm py-2 block font-semibold font-sans mt-3'>Læs mere</a>
                      }
                    </div>
                    {card.logo &&
                    <div className='flex items-start justify-start flex-col gap-y-2 mb-1'>
        ...
        <div className="block w-full mt-12 text-center">
          <div className='flex gap-x-4 justify-center p-2'>
            <button
              ...
              className={`hover:outline-none focus:outline-none hover:border-black/50 group flex cursor-pointer items-center space-x-3 rounded-full p-2 text-sm font-medium border z-30`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth="1.5" className='size-12 group-hover:stroke-black/50 stroke-black'>
